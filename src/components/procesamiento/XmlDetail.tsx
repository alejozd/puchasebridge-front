...
const XmlDetail: React.FC<XmlDetailProps> = ({
  ...
  const pendingCount = useMemo(
    () =>
      detail?.productos.filter((p) => p.estadoProducto !== "HOMOLOGADO")
        .length ?? 0,
    [detail?.productos],
  );
  ...
  const getStatusTag = (status: string) => {
    ...
    if (s === "VALIDADO" || s === "LISTO")
      return (
        <Tag value="VALIDADO" severity="info" className="status-tag-large" />
      );
    if (s === "ERROR")
      return (
        <Tag value="ERROR" severity="danger" className="status-tag-large" />
      );
    return (
      <Tag value="CARGADO" severity="warning" className="status-tag-large" />
    );
  };
            ...
            <React.Fragment key={event.label}>
              <div className="timeline-event-subtle">
                <div className={`event-marker ${event.done ? "done" : ""}`}>
                  <i
                    className={event.done ? "pi pi-check" : "pi pi-circle-on"}
                  />
                </div>
              ...
              {index < timelineEvents.length - 1 && (
                <div
                  className={`event-connector ${timelineEvents[index + 1].done ? "done" : ""}`}
                />
              )}
            </React.Fragment>
          ))}
      ...
      {generatedDoc && (
        <div className="generated-doc-simple p-3 border-round mb-3 flex align-items-center gap-3 bg-blue-50">
          ...
          <div className="flex-grow-1">
            <span className="block text-xs font-medium text-blue-700 mb-0">
              Documento en Helisa:
            </span>
            <span className="block text-sm font-mono font-semibold text-primary">
              {generatedDoc}
            </span>
          </div>
          <Button
        ...
        {detail.estado.toUpperCase() === "VALIDADO" && (
          <div className="flex align-items-center gap-3 w-full justify-content-between">
            ...
            <Button
              ...
              tooltip={
                !canProcess ? "Homologa todos los productos primero" : ""
              }
